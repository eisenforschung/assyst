...
def pool():
    """An ASSYST pool of Al structures labeled with the universal GRACE model, tagged by pipeline step."""
    ...
    volume_minima = list(tag_stage(relax(seeds, VolumeRelax(**settings), calculator), "volmin"))
    minima = list(tag_stage(relax(volume_minima, FullRelax(**settings), calculator), "allmin"))
    ...
    for structure in minima + rattled + stretched:
        stage = structure.info["stage"]
        structure = structure.copy()
        structure.info["stage"] = stage
...
def test_pool_is_labeled_and_diverse(pool, labels):
    ...
    assert {stage_of(s) for s in pool} == {"allmin", "rattle", "stretch"}
...
def test_relaxed_minima_are_redundant(pool, scores):
    ...
    stages = np.array([stage_of(s) for s in pool])
    assert scores[stages == "allmin"].mean() < scores[stages == "stretch"].mean(), \
        "Relaxed minima must carry less unique information than stretched structures!"
    ranked = np.argsort(scores)[::-1]
    assert stages[ranked[0]] != "allmin", "The single most informative structure must not be a relaxed minimum!"
...
def test_trace_accounts_for_every_structure(pool, scores):
    ...
    selected = select(pool, fraction=FRACTION, featurizer=FEATURIZER, rng=0)
    traced = trace(pool, selected, scores=scores)
    ...
    assert set(traced["stage"]) == {"allmin", "rattle", "stretch"}
...
def test_summary_balances(pool, scores):
    ...
    selected = select(pool, fraction=FRACTION, featurizer=FEATURIZER, rng=0)
    summary = summarize(trace(pool, selected, scores=scores))

    assert set(summary.index) == {"allmin", "rattle", "stretch"}
    ...
    assert summary.loc["allmin", "mean_score"] < summary.loc["stretch", "mean_score"], \
